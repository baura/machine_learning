...
```{r download_and_load_of_file, echo=T, warning=F, error=F}
#install.packages("adaptDA", "adabag", "mlbench", "earth", "mda")
library(caret)
...
```{r basic_info, echo=T, warning=F, error=F}
...
```
So the training set has `dim(training)[1]` observations and `dim(training)[2]` variables, with around 3000 observations per subject.
...
```{r exploratory_analysis, echo=T, warning=F, error=F}
...
```
I turned out that up to `length(too_na_cols)` were for 90% of the observations with not available data. I therefore preferred to kick them out of the training dataset. Checking for collinearity among the left variables might help.

```{r collinearity_check, echo=T, error=F, warning=F}
...
```
There are therefore `length(which(err==1))/2` pairs of variables that are collinear, but in prediction models this is not an issue. Anyway, for computational time purposes, it is still better to perform principal component analysis on the training subset 
and then run some classification predictive methods, like classification tree here below.
```{r first_model_pca_and_ctree, echo=T, error=F, warning=F}
...
confusionMatrix(test_train$classe, pred1)$table
accuracy1<-sum(diag(confusionMatrix(pred1, test_train$classe)$table))/sum(confusionMatrix(pred1, test_train$classe)$table)
accuracy1
```

Below here I try a predictor with PCA + linear discriminant analysis.
```{r first_model_pca_and_lda, echo=T, error=F, warning=F}
...
```{r first_model_pca_and_bagfda, echo=T, error=F, warning=F}
...
```{r first_model_pca_and_fda, echo=T, error=F, warning=F}
...
```{r first_model_pca_and_amdai, echo=T, error=F, warning=F}
...
Below here I try a predictor with deepboost and it doesn't work.
```{r first_model_pca_and_deepboost, echo=T, error=F, warning=F}
...
```{r first_model_pca_and_qda, echo=T, error=F, warning=F}
...
Below here I try a predictor with PCA+ quadratic discriminative analysis.
```{r first_model_pca_and_linda, echo=T, error=F, warning=F}
...
```{r first_model_pca_and_hda, echo=T, error=F, warning=F}
#modFit8<-train(classe ~ ., method="hda", data=trainPC, verbose=F) #it didn't work
modFit8<-train(classe ~ PC1+PC2+PC3+PC4 +PC5+PC6  + PC7 +PC8+PC9+PC10+PC11+PC12+PC13+PC14+PC15+PC16 +PC17+PC18+PC19+PC20, method="hda", data=trainPC, na.action=na.omit, verbose=F)
...
accuracy8  #too low, not to be included
```

```{r first_model_pca_and_hdda, echo=T, error=F, warning=F}
#modFit9<-train(classe ~ ., method="hdda", data=trainPC, verbose=F) 
...
```{r first_model_pca_and_lssvmlinear, echo=T, error=F, warning=F}
modFit10<-train(classe ~ ., method="lssvmLinear", data=trainPC, verbose=F)
#modFit10<-train(classe ~ PC1+PC2+PC3+PC4 +PC5+PC6  + PC7 +PC8+PC9+PC10+PC11+PC12+PC13+PC14+PC15+PC16 +PC17+PC18+PC19+PC20, method="loclda", data=trainPC, na.action=na.omit)
...
```{r first_model_pca_and_pda, echo=T, error=F, warning=F}
modFit11<-train(classe ~ ., method="pda", data=trainPC, verbose=F)
#modFit10<-train(classe ~ PC1+PC2+PC3+PC4 +PC5+PC6  + PC7 +PC8+PC9+PC10+PC11+PC12+PC13+PC14+PC15+PC16 +PC17+PC18+PC19+PC20, method="loclda", data=trainPC, na.action=na.omit)
...
```{r first_model_pca_and_pda, echo=T, error=F, warning=F}
modFit12<-train(classe ~ ., method="multinom", data=trainPC, verbose=F)
#modFit10<-train(classe ~ PC1+PC2+PC3+PC4 +PC5+PC6  + PC7 +PC8+PC9+PC10+PC11+PC12+PC13+PC14+PC15+PC16 +PC17+PC18+PC19+PC20, method="loclda", data=trainPC, na.action=na.omit)
...
```{r combined_model_via_stacking, echo=T, error=F, warning=F}
...
Now I will apply the same process in the testing dataset: principal component analysis, compute those predictors that proved enough accurate on *training*, combine them with the random forest process used above (*combModFit*).

```{r testing_phase, echo=T, warning=F, error=F,  results="hide"}
...
predictors_test<-cbind(predict(modFit1, newdata=PCtest), predict(modFit2, newdata=PCtest), predict(modFit4, newdata=PCtest), predict(modFit5, newdata=PCtest),predict(modFit7, newdata=PCtest), predict(modFit9, newdata=PCtest), predict(modFit11, newdata=PCtest), predict(modFit12, newdata=PCtest))
dim(predictors_test)
colnames(predictors_test)<-c("pred1", "pred2","pred4","pred5","pred7","pred9","pred11","pred12")
...
`as.character(levels(combPred))[combPred]`
